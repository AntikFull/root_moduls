	...
	if(globalArgs.splittcpsizepacket >= *len){
        ...
        for(int i = 0; i + 3 < *len; i++){
				...
				if(globalArgs.splittcp == 1){
					for(int n = i; n > 0; n--){
                                                if(n + 2 < *len && data[n] == '.' && (data[n+1]>='a'&&data[n+1]<='z') && (data[n+2]>='a'&&data[n+2]<='z') && (data[n-1]>='0'&&data[n-1]<='z')){
...
int splittcp(uint8_t *data, int len, uint32_t *mark, uint32_t index) {
	if(globalArgs.splittcp == 0)
		return 0;
	/* Не доверяем длине пакета из NFQUEUE: VLA и заголовки используют её напрямую. */
	if(data == NULL || len < (int)sizeof(struct iphdr))
		return 0;
	...
	if(len < iphdrl + (int)sizeof(struct tcphdr))
		return 0;
	...
	if(tcphdr->doff < 5 || len < iphdrl + tcphdrl)
		return 0;
