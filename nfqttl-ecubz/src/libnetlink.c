...
void rtnl_set_strict_dump(struct rtnl_handle_ *rth)
{
	int one = 1;
...
int rtnl_add_nl_group(struct rtnl_handle_ *rth, unsigned int group)
{
	return setsockopt(rth->fd, SOL_NETLINK, NETLINK_ADD_MEMBERSHIP,
			  &group, sizeof(group));
}

void rtnl_close_(struct rtnl_handle_ *rth)
{
	if (rth->fd >= 0) {
		close(rth->fd);
		rth->fd = -1;
...
int rtnl_open_byproto(struct rtnl_handle_ *rth, unsigned int subscriptions,
		      int protocol)
{
	socklen_t addr_len;
	int sndbuf = 32768;
...
int rtnl_open_(struct rtnl_handle_ *rth, unsigned int subscriptions)
{
	return rtnl_open_byproto(rth, subscriptions, NETLINK_ROUTE);
}

int rtnl_nexthopdump_req(struct rtnl_handle_ *rth, int family,
...
int rtnl_addrdump_req(struct rtnl_handle_ *rth, int family,
		      req_filter_fn_t filter_fn)
{
	struct {
		struct nlmsghdr nlh;
...
int rtnl_addrlbldump_req(struct rtnl_handle_ *rth, int family)
{
	struct {
		struct nlmsghdr nlh;
...
int rtnl_routedump_req(struct rtnl_handle_ *rth, int family,
		       req_filter_fn_t filter_fn)
{
	struct {
		struct nlmsghdr nlh;
...
int rtnl_ruledump_req(struct rtnl_handle_ *rth, int family)
{
	struct {
		struct nlmsghdr nlh;
...
int rtnl_neighdump_req(struct rtnl_handle_ *rth, int family,
		       req_filter_fn_t filter_fn)
{
	struct {
		struct nlmsghdr nlh;
...
int rtnl_neightbldump_req(struct rtnl_handle_ *rth, int family)
{
	struct {
		struct nlmsghdr nlh;
...
int rtnl_mdbdump_req(struct rtnl_handle_ *rth, int family)
{
	struct {
		struct nlmsghdr nlh;
...
int rtnl_netconfdump_req(struct rtnl_handle_ *rth, int family)
{
	struct {
		struct nlmsghdr nlh;
...
int rtnl_nsiddump_req_filter_fn(struct rtnl_handle_ *rth, int family,
				req_filter_fn_t filter_fn)
{
	struct {
		struct nlmsghdr nlh;
...
static int __rtnl_linkdump_req(struct rtnl_handle_ *rth, int family)
{
	struct {
		struct nlmsghdr nlh;
...
int rtnl_linkdump_req(struct rtnl_handle_ *rth, int family)
{
	if (family == AF_UNSPEC)
...
int rtnl_linkdump_req_filter(struct rtnl_handle_ *rth, int family,
			    __u32 filt_mask)
{
...
int rtnl_linkdump_req_filter_fn(struct rtnl_handle_ *rth, int family,
				req_filter_fn_t filter_fn)
{
...
int rtnl_fdb_linkdump_req_filter_fn(struct rtnl_handle_ *rth,
				    req_filter_fn_t filter_fn)
{
	struct {
		struct nlmsghdr nlh;
...
int rtnl_statsdump_req_filter(struct rtnl_handle_ *rth, int fam, __u32 filt_mask)
{
	struct {
		struct nlmsghdr nlh;
...
int rtnl_send(struct rtnl_handle_ *rth, const void *buf, int len)
{
	return send(rth->fd, buf, len, 0);
}

int rtnl_send_check(struct rtnl_handle_ *rth, const void *buf, int len)
{
	struct nlmsghdr *h;
	int status;
	char resp[1024];
...
int rtnl_dump_request(struct rtnl_handle_ *rth, int type, void *req, int len)
{
	struct nlmsghdr nlh = {
...
int rtnl_dump_request_n(struct rtnl_handle_ *rth, struct nlmsghdr *n)
{
	struct sockaddr_nl nladdr = { .nl_family = AF_NETLINK };
	struct iovec iov = {
		.iov_base = n,
...
static void rtnl_dump_error(const struct rtnl_handle_ *rth,
			    struct nlmsghdr *h)
{
...
static int rtnl_dump_filter_l(struct rtnl_handle_ *rth,
...
int rtnl_dump_filter_nc(struct rtnl_handle_ *rth,
...
static int __rtnl_talk_iov(struct rtnl_handle_ *rtnl, struct iovec *iov,
...
static int __rtnl_talk(struct rtnl_handle_ *rtnl, struct nlmsghdr *n,
...
int rtnl_talk(struct rtnl_handle_ *rtnl, struct nlmsghdr *n,
	      struct nlmsghdr **answer)
{
	return __rtnl_talk(rtnl, n, answer, true, NULL);
}

int rtnl_talk_iov(struct rtnl_handle_ *rtnl, struct iovec *iovec, size_t iovlen,
		  struct nlmsghdr **answer)
{
	return __rtnl_talk_iov(rtnl, iovec, iovlen, answer, true, NULL);
}

int rtnl_talk_suppress_rtnl_errmsg(struct rtnl_handle_ *rtnl, struct nlmsghdr *n,
				   struct nlmsghdr **answer)
{
	return __rtnl_talk(rtnl, n, answer, false, NULL);
}

int rtnl_listen_all_nsid(struct rtnl_handle_ *rth)
{
	unsigned int on = 1;
...
int rtnl_listen(struct rtnl_handle_ *rtnl,
...
int addraw_l(struct nlmsghdr *n, int maxlen, const void *data, int len)
{
	...
	memcpy(NLMSG_TAIL_(n), data, len);
	memset((void *) NLMSG_TAIL_(n) + len, 0, NLMSG_ALIGN(len) - len);
...
{
	struct rtattr *nest = NLMSG_TAIL_(n);
...
{
	nest->rta_len = (void *)NLMSG_TAIL_(n) - (void *)nest;
	return n->nlmsg_len;
}
...
{
	struct rtattr *start = NLMSG_TAIL_(n);
...
int addattr_nest_compat_end(struct nlmsghdr *n, struct rtattr *start)
{
	...
	start->rta_len = (void *)NLMSG_TAIL_(n) - (void *)start;
