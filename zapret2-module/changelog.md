...
* Список приложений помещён в отдельную прокручиваемую область с ограничением высоты под экран телефона.
* Поиск и фильтры остаются сверху, длинный перечень больше не растягивает всю страницу.

# v2.3.0

* Исправлен список приложений: больше не используется хрупкий разбор вывода `pm list packages -U` через `awk`.
* Приложения загружаются через API KernelSU `listPackages` и `getPackagesInfo` с пакетной обработкой.
* Добавлены чекбоксы «Пользовательские» и «Системные».
* Добавлен поиск по названию приложения и package name.
* В строках показывается название приложения, иконка, package name и тип приложения.
* Чекбоксы INCLUDE/EXCLUDE отражают текущие списки и сохраняют изменения сразу.

# v2.2.0

* WebUI переведён на нативный KernelSU WebUI API `ksu.exec` по официальной архитектуре KernelSU.
* Удалён отдельный BusyBox httpd на `127.0.0.1` и shell CGI, из-за которого WebView иногда получал исходник скрипта вместо JSON.
* Управление статусом, настройками, приложениями, списками и журналом теперь работает непосредственно из страницы модуля KernelSU Next.

# v2.1.2

* WebUI автоматически повторяет запросы статуса и списка приложений после временной недоступности CGI.
* Вместо ошибки разбора `#!/system/bin/sh` показывается понятное сообщение о восстановлении соединения.
* После восстановления CGI интерфейс обновляется без ручной перезагрузки страницы.

# v2.1.1

* Исправлено зависание WebUI на надписи «Загрузка статуса» в браузерах Android.
* Кнопки показывают ход операции, результат и ошибки CGI; ответы больше не кэшируются.
* Настройки применяются одним reload вместо трёх последовательных перезапусков.
* Исправлен список приложений с путями APK, содержащими символ `=`.
* Добавлена сериализация reload, очистка осиротевших процессов nfqws2 и дублирующихся правил iptables.
* После запуска проверяется, что nfqws2 действительно занял очередь NFQUEUE.

# v2.1.0

* Добавлен рабочий локальный WebUI на `127.0.0.1:18080` с управлением режимами, приложениями, списками и журналом.
* Добавлены `QUIC_MODE` (`OFF`, `SELECTED`, `GLOBAL`) и безопасный режим `SELECTED` по умолчанию.
* Авто-список доменов теперь применяется к дополнительной стратегии `DESYNC_ARGS_ALT`.
* Служба использует PID-файлы вместо остановки процессов других модулей через `pkill`.
