...
setup_private_route_table() {
  ...
  valid_number "$VPN_ROUTE_TABLE" || { log "РРРРССРРСРСР VPN_ROUTE_TABLE=$VPN_ROUTE_TABLE"; return 1; }
  ...
  "$IP_BIN" -4 route flush table "$VPN_ROUTE_TABLE" 2>/dev/null || true
  "$IP_BIN" -6 route flush table "$VPN_ROUTE_TABLE" 2>/dev/null || true
  [ "$has4" = 1 ] || { log "VPN=$vpn_if РР РРРРС IPv4-РРСРСР; IPv4 VPN sharing РРРРСССРРР"; return 1; }
  "$IP_BIN" -4 route replace default dev "$vpn_if" table "$VPN_ROUTE_TABLE" 2>/dev/null || { log "РР СРРРРСС СРРРРСС IPv4 default dev $vpn_if table $VPN_ROUTE_TABLE"; return 1; }
  if [ "$has6" = 1 ]; then
    "$IP_BIN" -6 route replace default dev "$vpn_if" table "$VPN_ROUTE_TABLE" 2>/dev/null || log "IPv6 default СРСРР $vpn_if РР СРРРРР; leak guard РРРРРССРС СРРРСРСРРР РССРР"
  fi
  return 0
}
...
setup_guard_only() {
  ...
  log "fallback=BLOCK: VPN РР РРСРР, РРСРСРРС РРРРРСРР СРРРРСР РРРРРРРСРРРР"
}

setup_vpn_nat_forward_guard() {
  local vpn_if="$1" downs="$2" down subnet
  [ -x "$IPT" ] || { log "iptables РРРРСССРРР: VPN sharing РР РСРРРРСР"; return 1; }
  "$IPT" -w 5 -t filter -N ZAPRET2_VPN_FORWARD >/dev/null 2>&1 || return 1
  "$IPT" -w 5 -t filter -I FORWARD 1 -j ZAPRET2_VPN_FORWARD >/dev/null 2>&1 || return 1
  ...
  for down in $downs; do
    ...
    if [ "$VPN_HOTSPOT_MASQUERADE" = "1" ]; then
      subnet=$(subnet_for_iface "$down"); [ -n "$subnet" ] || { log "РР СРРРРСС РРСРРРРРСС РРРСРСС downstream=$down РРС MASQUERADE"; return 1; }
...
apply_rules() {
  ...
  [ -x "$IP_BIN" ] || { log "РРРРРРР ip РР РРРРРРР"; return 1; }
  ...
  if [ -z "$downs" ]; then
    if downstream_candidate_exists; then log "downstream РРСРРРСС, РР РСС РР РРСРР; РРРСРСРР"; return 3; fi
    log "VPN sharing РРРСССР, РР РРСРРРСР tether downstream РРРР РР РРРРРР"
  ...
  vpn_if=$(detect_vpn_iface)
  if [ -z "$vpn_if" ]; then
    if fallback_is_block; then setup_guard_only "$downs" || return 1; log "VPN РРРРСССРРР: fallback=BLOCK"; else log "VPN РРРРСССРРР: fallback=ANTIDPI, РРССРСР upstream + Zapret2"; fi
    if vpn_candidate_exists; then log "VPN-РРРРРРРС РСС Р РРСРСРРРРР СРССРСРРР"; return 3; fi
    atomic_meta_write "$sig"
    return 0
  fi
  ...
  for down in $downs; do
    pref=$(next_free_pref "$pref") || { log "РРС СРРРРРРРРР policy priority"; cleanup_rules; fallback_is_block && setup_guard_only "$downs"; return 1; }
    add_policy_rule "$pref" "$down" || { log "РР СРРРРСС РРРРРРСС policy rule РРС $down"; cleanup_rules; fallback_is_block && setup_guard_only "$downs"; return 1; }
    pref=$((pref + 1))
  done
  setup_vpn_nat_forward_guard "$vpn_if" "$downs" || { log "РР СРРРРСС РРСССРРСС VPN NAT/FORWARD/guard"; cleanup_rules; fallback_is_block && setup_guard_only "$downs"; return 1; }
