...
for kv in 'COMPONENT_MODE=SAFE' 'COMPONENT_BACKEND=PM' 'AD_SURFACE_KILLER=1' 'AD_KILLER_FORCE_TCP=0' 'MAX_IFW_ACTIVITIES_PER_CATEGORY=5' 'PACKAGE_POLL_INTERVAL=60' 'PACKAGE_SAFETY_POLL_INTERVAL=900' 'BOOT_STABILIZATION=1' 'BOOT_STABILIZATION_MIN_UPTIME_SEC=120' 'BOOT_STABILIZATION_MAX_WAIT_SEC=300' 'BOOT_STABILIZATION_SAMPLE_SEC=5' 'BOOT_STABILIZATION_REQUIRED_SAMPLES=3' 'BOOT_STABILIZATION_MIN_IDLE_PERCENT=60' 'BOOT_STABILIZATION_MIN_AVAILABLE_KB=1048576' 'AD_SURFACE_WORKERS=4' 'AD_SURFACE_MAX_RUNTIME_SEC=1800' 'AD_SURFACE_MAX_APKS_PER_PACKAGE=64' 'AD_SURFACE_RERUN_DELAY_SEC=60' 'AD_SURFACE_FOUR_WORKER_MIN_AVAILABLE_KB=4194304' 'AD_SURFACE_THREE_WORKER_MIN_AVAILABLE_KB=2097152' 'AD_SURFACE_MIN_AVAILABLE_KB=1048576' 'MAX_AGGRESSIVE_ADS_MATCHES=64' 'MAX_AGGRESSIVE_IFW_ACTIVITIES_PER_CATEGORY=64' 'BLOCK_PUSH_SDK=0' 'AD_KILLER_MODE=auto' 'AD_KILLER_IP_FALLBACK=0' 'AD_KILLER_MIN_CONFIDENCE=CAPABILITY' 'LOG_MIRROR=1' 'LOG_MIRROR_FULL=0' 'LOG_MIRROR_INTERVAL=60' 'XPOSED_BRIDGE=auto'; do
  key=${kv%%=*}
...
# v5.1.0 использовал стандартный предел в два воркера. При обновлении переводим
# это прежнее значение на новый стандарт; остальные пользовательские значения сохраняем.
if grep -q '^AD_SURFACE_WORKERS=2$' "$SETTINGS_FILE" 2>/dev/null; then
  settings_tmp="$SETTINGS_FILE.tmp.$$"
  sed 's/^AD_SURFACE_WORKERS=2$/AD_SURFACE_WORKERS=4/' "$SETTINGS_FILE" > "$settings_tmp" \
    && mv -f "$settings_tmp" "$SETTINGS_FILE"
  rm -f "$settings_tmp" 2>/dev/null
fi
