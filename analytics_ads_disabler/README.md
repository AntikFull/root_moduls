# Analytics & Ads Disabler v6.1.1

> **v6.1.1:** Устранение ошибки `uid_inventory_failed` и зацикливания `SIDE-EFFECTS-PENDING` в Ad Killer через прямое чтение `packages.list`, отказоустойчивое построение инвентаря путей APK для Multi-User профилей и безопасная проверка целостности пакета перед установкой.
